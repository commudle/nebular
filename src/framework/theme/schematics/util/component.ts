...
export function getComponentTemplateDescriptor(host: Tree, componentPath: string): TemplateDescriptor {
  ...
  const compMetadata: any = getDecoratorMetadata(compSource, 'Component', '@angular/core')[0];
...
function getMetadataProperty(metadata: any, propertyName: string): ts.PropertyAssignment {
